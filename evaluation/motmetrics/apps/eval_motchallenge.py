...
string.""", formatter_class=argparse.RawTextHelpFormatter)
    ...
    parser.add_argument('--groundtruths', type=str, default='yourdata',
                        help='Directory containing ground truth sub-folders (each with gt/gt.txt)')
    parser.add_argument('--tests', type=str, default='yourdata',
                        help='Directory containing tracker result .txt files')
...
def main():
    ...
    # min_confidence=-1 means accept all tracks regardless of confidence score.
    # Change this if your tracker outputs meaningful confidence values.
    ...
    # iou_threshold=0 → use Euclidean centre-point distance (dist_threshold in pixels).
    # Set iou_threshold to a positive value (e.g. 0.5) to switch to IoU-based matching.
    iou_threshold = 0.0
    dist_threshold = 5000   # pixels; adjust to your image resolution
